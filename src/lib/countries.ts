/** ISO 3166-1 alpha-2 countries for profile country picker. */
...
/** Regional-indicator flag emoji from ISO alpha-2 (e.g. "US" → 🇺🇸). */
...
/** "🇧🇷 Latam · Profile name" for interview cards / columns. */
export function formatProfileLabel(
  ...
  const prefix = [flag, regionPart].filter(Boolean).join(' ');
  return prefix ? `${prefix} · ${label}` : label;
}

export const PROFILE_REGIONS = ['Latam', 'EU', 'Canada', 'US', 'Asia'] as const;
export type ProfileRegion = (typeof PROFILE_REGIONS)[number];

export const PROFILE_REGION_OPTIONS = [
  { value: '', label: 'None' },
  ...PROFILE_REGIONS.map((r) => ({ value: r, label: r })),
];
