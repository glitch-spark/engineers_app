...
type Acc = { _id: string; name: string; email: string; phone?: string; address?: string; ownerName?: string };
...
export default function AccountsPage() {
  ...
  const save = async () => {
    try {
      if (editing) await api.updateAccount(editing._id, form);
      else await api.createAccount(form);
      await mutate();
      setOpen(false);
    } catch {
      /* error already surfaced via apiFetch on 401; let other errors fall through silently to keep parity */
    }
  };
